...
interface TechDebtManagerProps {
  onDataChanged: () => void;
}

export default function TechDebtManager({ onDataChanged }: TechDebtManagerProps) {
  ...
  const handleFixTechDebt = (id: string, hours: string) => {
    const val = parseFloat(hours) || 0;
    fixBug(id, val);
    setBugs(loadBugs());
    onDataChanged();
    toast.success("Débito técnico resolvido!");
  };

  const handleDelete = (id: string) => {
    if (confirm("Tem certeza que deseja excluir este débito técnico?")) {
      deleteBug(id);
      setBugs(loadBugs());
      onDataChanged();
      toast.success("Débito técnico removido!");
    }
  };
  ...
  if (bugs.length === 0) {
    return <div className="text-center p-8 text-gray-500">Nenhum débito técnico encontrado.</div>;
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-gray-100 text-left">
            <th className="py-3 px-2">Débito Técnico</th>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        ...
                        handleFixTechDebt(b.id, input.value);
                      }}
