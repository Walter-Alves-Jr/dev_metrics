...
export default function IncidentsManager({ onDataChanged }: IncidentsManagerProps) {
  ...
  if (incidents.length === 0) {
    return <div className="text-center p-8 text-gray-500">Nenhum incidente encontrado.</div>;
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-gray-100 text-left">
            <th className="py-3 px-2">Incidente</th>
              ...
              <td className="py-3 px-2">
                <div className="flex flex-col gap-1">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold w-fit ${
                    ...
                    {i.resolvedAt ? "Resolvido" : i.acknowledgedAt ? "Reconhecido" : "Aberto"}
                  </span>
                  {i.resolvedAt && (
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold w-fit ${
